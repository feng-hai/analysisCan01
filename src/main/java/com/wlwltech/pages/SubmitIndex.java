...
import simplekafka08.simplekafka08.publicStaticMap;
	...
	public void run() {
		...
		try {
			...
			for (VehicleIndex vehicle : vehicles) {
				...
				if(System.currentTimeMillis()-vehicle.getTime()>3000000)
				{
			
					System.out.println(vehicle.getVehicleUnid() + "---" + sdf.format(new Date(vehicle.getTime())));
				}
				bulkRequest.add(EsutilSingle.getClient().prepareIndex(tableName, type)
						.setId(vehicle.getVehicleUnid() + "").setSource(hashMap));

			}
			// logger.error("开始提交02");
			bulkRequest.execute().actionGet();
			// logger.error("开始提交03");
			System.out.println(publicStaticMap.getQueueES().size() + "完成时间" + (System.currentTimeMillis() - start));
			// logger.error("开始结束");
		} catch (Exception ex) {
